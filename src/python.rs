...
pub fn build_python(code_block: String, sandbox_args_vec: Vec<String>) -> String{
    ...
    sandbox_args_vec.push("python".to_string());
    ...
    let result = remove_dir_from_error(&result, "input.py".to_string());
...
fn build_python_test(){
    ...
    let code_block="print('Hello Python')";
    let result = build_python(code_block.to_string(), sandbox_args_vec);
    println!("{}", result);

    assert!(result.eq("Hello Python\n"));
}